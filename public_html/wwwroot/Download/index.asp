...
titlefl = "Download center"

else

set rs2=server.createobject("adodb.recordset") 
exec2="select * from download_fl where id="&id&" order by id desc  " 
...
<!--#include file="../Inc/left5.asp"-->
    ...
    <div id="right">   
        <div class="category_title">Your position：<a href="<%=root%>/index.html">Home</a> > <%=titlefl%></div>
            ...
            For i=1 To x
            %>
                <li><div class="list_time"><%=rs("data")%></div><a href="<%=rs("id")%>.html" title="<%=rs("title")%>"><%=left(rs("title"),80)%></a></li>
            <% 
            rs.movenext 
            next
            end if
