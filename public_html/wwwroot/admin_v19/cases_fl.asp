...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" bgcolor="#F7F7F7"><div><strong>案例分类管理</strong><span class="text2"></span></div></td>
  </tr>
  <tr>
<td bgcolor="#FFFFFF">
            <%	
set rs=server.createobject("adodb.recordset") 
exec="select * from cases_fl order by px_id asc" 
...
%>
<%For i=1 To x%>
<form  name="add" method="post" action="cases_fl.asp?xiugaipx=ok">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        <tr>
          <td width="27%" bgcolor="#F7F7F7"><input name="id" type="hidden" id="id" value="<%=rs("id")%>" />
            <label>
            分类名称：
            <input name="title" type="text" value="<%=rs("title")%>">
            </label></td>
          <td width="13%" bgcolor="#F7F7F7">排序： 
            <input name="px_id" type="text" value="<%=rs("px_id")%>" size="2"></td>
          <td width="13%" bgcolor="#F7F7F7"><label>
            <input type="submit" name="button" id="button" value="修改分类">
          </label></td>
          <td width="47%" bgcolor="#F7F7F7"><input type="button" name="Submit" value="删除分类" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='cases_fl.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
</form>
      <%rs.movenext
...
<table width="95%" border="0" align="center" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
  ...
  <tr>
    <td bgcolor="#FFFFFF"><form name="form1" method="post" action="cases_fl.asp?xiugaifl=ok">
      中文名称：
...
id=request("id") 
sql="select * from cases_fl where id="&id 
set rs=server.CreateObject("adodb.recordset")
...
rs.update 
rs.close 
response.Write("<script language=""javascript"">alert(""修改分类成功！"");window.location.href='cases_fl.asp';</script>")
end if
%> 
...
set rs=server.createobject("adodb.recordset")
sql="select title,px_id from cases_fl"
rs.open sql,conn,1,3
...
conn.close
set rs=nothing
Response.Write "<script>alert('分类增加成功！');window.location.href='cases_fl.asp';</script>" 
end if
%>
...
rs.delete
rs.update
Response.Write "<script>alert('分类刪除成功！');window.location.href='cases_fl.asp';</script>" 
end if
%>
