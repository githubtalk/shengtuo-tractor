...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" bgcolor="#F7F7F7"><div><strong>下载分类管理</strong><span class="text2"></span></div></td>
  </tr>
  <tr>
<td bgcolor="#FFFFFF">
<%	
...
%>
<%For i=1 To x%>
<form  name="add" method="post" action="download_fl.asp?xiugaipx=ok">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="45%" bgcolor="#FFFFFF"><input type="button" name="Submit" value="删除分类" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='download_fl.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
</form>
      <%rs.movenext
...
<table width="95%" border="0" align="center" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
  ...
  <tr>
    <td bgcolor="#FFFFFF"><form name="form1" method="post" action="download_fl.asp?xiugaifl=ok">
      中文名称：
...
rs.update 
rs.close 
response.Write("<script language=""javascript"">alert(""修改分类成功！"");window.location.href='download_fl.asp';</script>")
end if
%> 
<%
if Request.QueryString("xiugaifl")="ok" then
set rs=server.createobject("adodb.recordset")
sql="select title,px_id from download_fl"
rs.open sql,conn,1,3
...
conn.close
set rs=nothing
Response.Write "<script>alert('分类增加成功！');window.location.href='download_fl.asp';</script>" 
end if
%>
<%
...
rs.delete
rs.update
Response.Write "<script>alert('分类刪除成功！');window.location.href='download_fl.asp';</script>" 
end if
%>
