...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" bgcolor="#F7F7F7"><div><strong>新闻分类管理</strong><span class="text2"></span> （<font color="#0000FF">"公司新闻"请勿删除，否则前台的首页公司新闻将不会显示!</font>）</div></td>
  </tr>
  <tr>
<td bgcolor="#FFFFFF">
            <%	
...
<form  name="add" method="post" action="news_fl.asp?xiugaipx=ok">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        <tr>
          <td width="28%" bgcolor="#F7F7F7"><input name="id" type="hidden" id="id" value="<%=rs("id")%>" />
            <label>
            分类名称：
          ...
          <td width="46%" bgcolor="#F7F7F7"><input type="button" name="Submit" value="删除分类" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='news_fl.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
</form>
      <%rs.movenext
...
set rs=server.createobject("adodb.recordset")
sql="select title,px_id from news_fl"
rs.open sql,conn,1,3
