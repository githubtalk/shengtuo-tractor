...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" bgcolor="#F7F7F7"><div><strong>产品小类管理</strong><span class="text2"></span></div></td>
  </tr>
  <tr>
<td bgcolor="#FFFFFF">
<%
...
exec="select * from shop_fl where sid = "&sid&" order by px_id asc" 
rs.open exec,conn,1,1
if rs.eof then
response.Write "&nbsp;暂无此小类！"
else
...
%>
<%For i=1 To x%>
<form  name="add" method="post" action="shop_flx.asp?sid=<%=sid%>&xiugaipx=ok">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="53%" bgcolor="#FFFFFF"><input type="button" name="Submit" value="删除分类" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='shop_flx.asp?act=del&id=<%=rs("id")%>&sid=<%=sid%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
</form>
      <%rs.movenext
...
<table width="95%" border="0" align="center" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td bgcolor="#F7F7F7"><font color="#FF0000"><strong>增加小类</strong></font></td>
  </tr>
  <tr>
    <td bgcolor="#FFFFFF"><form name="form1" method="post" action="shop_flx.asp?sid=<%=sid%>&xiugaifl=ok">
      中文名称：
        <input name="title" type="text">
      排序：
      <input name="px_id" type="text" size="2"> 
        <input type="submit" name="button2" id="button2" value="增加小类">
...
rs.update 
rs.close 
response.Write("<script language=""javascript"">alert(""修改分类成功！"");window.location.href='shop_flx.asp?sid="&sid&"'</script>")
end if
%> 
...
set rs=server.createobject("adodb.recordset")
sql="select title,px_id,sid from shop_fl"
rs.open sql,conn,1,3
...
conn.close
set rs=nothing
Response.Write "<script>alert('分类增加成功！');window.location.href='shop_flx.asp?sid="&sid&"';</script>" 
end if
%>
...
rs.delete
rs.update
Response.Write "<script>alert('分类刪除成功！');window.location.href='shop_flx.asp?sid="&sid&"';</script>" 
end if
%>
