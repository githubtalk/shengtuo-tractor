      ...
      <table width="100%" border="1" align="center" cellpadding="6" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#F7F7F7">
        ...
        <tr>
		  ...
		  sid = rsc("id")
			...
			while not rscx.eof
			if rs("ssfl")=rscx("id") then
			response.Write("<option value=""" & rscx("id") & """ selected>" &"&nbsp;&nbsp;└"& rscx("title") & "</option>")
			else
		...
		set rsc=nothing
          ...
          &nbsp;&nbsp;&nbsp;产品排序：<input name="px_id" type="text" value="<%=rs("px_id")%>" size="2">
           <font color="#FF0000">（数字越小越靠前。）</font>
           &nbsp;&nbsp;&nbsp;首页显示：<input name="homeshow" type="checkbox" id="homeshow" value="1" <%if homeshow = 1 then response.Write "checked" end if%>>
        ...
        <tr>
          ...
          <td bgcolor="#F7F7F7"><label>
            <input type="text" name="price" id="price" value="<%=rs("price")%>" size="25" maxlength="50">
          </label></td>
        </tr>
        <tr>
