...
<%
dim id,rs,exec,rstt,exectt,tt,tt2
...
titlefl = "Successful Cases"

else

set rstt=server.createobject("adodb.recordset") 
exectt="select * from cases_fl where id="&id
...
conn.close
set conn = nothing
response.write "<script>alert('不存在此分类');history.back();</script>"
response.end
end if

set rs=server.createobject("adodb.recordset") 
exec="select * from [cases] where ssfl="&id&" order by id desc" 
rs.open exec,conn,1,1
...
<!--#include file="../Inc/left4.asp"-->
    ...
    <div id="right">   
        <div class="category_title">Your position：<a href="<%=root%>/index.html">Home</a> > <%=titlefl%></div>
            ...
            <%For i=1 To x%>
                <li>
                    <a href="<%=rs("id")%>.html" title="<%=CSQL(rs("title"))%>"><img name='picautozoom' class="img_border" src='<%=root%><%=rs("img")%>' width='155' height='125' /></a>
                    <p><a href="<%=rs("id")%>.html" title="<%=CSQL(rs("title"))%>"><%=left(rs("title"),50)%></a></p>
                </li>
            <%
            rs.movenext
            next
